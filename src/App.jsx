...
function EntryModal({ onClose, onSave, isos, editing }) {
  ...
  return (
    <ModalShell title={editing ? "EDITAR SOLDADURA" : "NUEVA SOLDADURA"} onClose={onClose}>
      <div className="grid grid-cols-2 gap-3">
      ...
      <button onClick={submit} className={btnPrimaryFull}>{editing ? "GUARDAR CAMBIOS" : "REGISTRAR SOLDADURA"}</button>
    </ModalShell>
  );
}
                          ...
                          <tr key={e.id} className={`hover:bg-slate-50 ${insp?.status === "rejected" ? "bg-red-50/50" : insp?.status === "accepted" ? "bg-emerald-50/30" : ""}`}>
                            ...
                            {canEdit && (role === "admin" || log.welderId === currentUser.id) && (
                              <td className="px-2 py-2">
                                <div className="flex gap-1">
                                  <button onClick={() => setEditingEntry({ ...e, logId: log.id })} className="text-slate-400 hover:text-blue-600 p-1 rounded hover:bg-blue-50" title="Editar"><Edit2 size={12} /></button>
                                  <button onClick={() => onDeleteEntry(log.id, e.id)} className="text-slate-400 hover:text-red-500 p-1 rounded hover:bg-red-50" title="Eliminar"><Trash2 size={12} /></button>
                                </div>
                              </td>
                            )}
                          </tr>
